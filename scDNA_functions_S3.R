...
#normalize####
#normalize divides a vector by its mean or median. there is an option to log2 transform it.
...
build_scDNAobj <- function(h5, count_matrix, cells_meta, bins_meta, transpose_count_matrix = FALSE, mean_read_length, genome_size){
...
calc_cells_ICF <- function(scDNAobj, ICF_loess_span = 0.75, max_chromo = 5){
  ...
  cells_meta <- cells_meta %>%
    ...
    mutate(model = loess(raw_ICF_score ~ ICCV)$fitted) %>%
    mutate(ICF_score = raw_ICF_score * median(model)/model)
